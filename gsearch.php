    ...
    if (!empty($gid)) {
        ...
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                $searchResult = "<br><br><br><table><tr><th>Gallery ID</th><th>Gallery Name</th><th>Location</th></tr>";
                while($row = $result->fetch_assoc()) {
                    $searchResult .= "<tr><td>" . htmlspecialchars($row["gid"]) . "</td><td>" . htmlspecialchars($row["gname"]) . "</td><td>" . htmlspecialchars($row["location"]) . "</td></tr>";
                }
                $searchResult .= "</table>";
            } else {
                $searchResult = "<span><br><br>Search Unsuccessful! Please Enter Correct Gallery ID.</span>";
            }
            $stmt->close();
        }
    }
...
?>
<html>
<head><title>Search Gallery</title></head>
<style>
...
<body style="background-color:aliceblue">
    <h1><center><font style="border:9px solid grey" face="arial">SEARCH FROM GALLERY TABLE</font></center></h1>
	<form action="gsearch.php" method="post">
		<div>Enter Gallery ID:<input type="text" name="gid"><br></div>
		<br><br>
		<button type="submit" class="btn">SEARCH</button>
	</form>
<?php if (!empty($gid)): ?>
	<b><br>Entered Gallery ID is <?php echo htmlspecialchars($gid); ?> <br></b>
<?php endif; ?>
