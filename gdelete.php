...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['gid'])) {
    $gid = trim($_POST['gid']);
    if (empty($gid)) {
        $message = "Error: Gallery ID cannot be empty";
        $messageClass = "error";
    } else {
        $checkSql = "SELECT * FROM gallery WHERE gid = ?";
        $checkStmt = $conn->prepare($checkSql);
        if ($checkStmt) {
            $checkStmt->bind_param("s", $gid);
            $checkStmt->execute();
            $result = $checkStmt->get_result();
            if ($result->num_rows > 0) {
                $deleteSql = "DELETE FROM gallery WHERE gid = ?";
                $deleteStmt = $conn->prepare($deleteSql);
                if ($deleteStmt) {
                    $deleteStmt->bind_param("s", $gid);
                    if ($deleteStmt->execute()) {
                        $message = "Record with Gallery ID = " . htmlspecialchars($gid) . " is deleted successfully.";
                ...
                $message = "Record '" . htmlspecialchars($gid) . "' was not found in database.";
                $messageClass = "error";
            }
            $checkStmt->close();
        }
...
?>
<html>
<head><title>Delete from Gallery</title></head>
<style>
...
<body style="background-color:lavenderblush">
    ...
    <form action="gdelete.php" method="POST">
        <div>Enter Gallery ID:<input type="text" name="gid" required><br></div>
        <br><br>
